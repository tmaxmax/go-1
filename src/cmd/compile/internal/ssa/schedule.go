...
func schedule(f *Func) {
	...
	for _, b := range f.Blocks {
		...
		sort.Slice(edges, func { i, j | return edges[i].x.ID < edges[j].x.ID })
		...
		for priq.Len() > 0 {
			...
			i := sort.Search(len(edges), func { i | return edges[i].x.ID >= v.ID })
			j := sort.Search(len(edges), func { i | return edges[i].x.ID > v.ID })
