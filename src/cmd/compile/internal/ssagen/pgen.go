...
func (s *ssafn) AllocFrame(f *ssa.Func) {
	...
	sort.SliceStable(fn.Dcl, func { i, j -> return cmpstackvarlt(fn.Dcl[i], fn.Dcl[j], mls) })
...
func fieldtrack(fnsym *obj.LSym, tracked map[*obj.LSym]struct{}) {
	...
	sort.Slice(trackSyms, func { i, j -> return trackSyms[i].Name < trackSyms[j].Name })
...
func CheckLargeStacks() {
	...
	sort.Slice(largeStackFrames, func { i, j -> return largeStackFrames[i].pos.Before(largeStackFrames[j].pos) })
