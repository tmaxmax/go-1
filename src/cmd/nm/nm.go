...
func nm(file string) {
	...
	for _, e := range entries {
		...
		switch *sortOrder {
		case "address":
			sort.Slice(syms, func { i, j -> return syms[i].Addr < syms[j].Addr })
		case "name":
			sort.Slice(syms, func { i, j -> return syms[i].Name < syms[j].Name })
		case "size":
			sort.Slice(syms, func { i, j -> return syms[i].Size > syms[j].Size })
		}
