...
func (ctxt *Link) inittaskSym(rootNames []string, symName string) loader.Sym {
	...
	sort.Slice(edges, func { i, j -> return edges[i].to < edges[j].to })
	...
	for !h.empty() {
		...
		a := sort.Search(len(edges), func { i -> return edges[i].to >= s })
		b := sort.Search(len(edges), func { i -> return edges[i].to > s })
