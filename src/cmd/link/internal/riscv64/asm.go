...
func findHI20Reloc(ldr *loader.Loader, s loader.Sym, val int64) *loader.Reloc {
	...
	start := sort.Search(relocs.Count(), func { i -> return ldr.SymValue(outer)+int64(relocs.At(i).Off()) >= val })
...
func findHI20Symbol(ctxt *ld.Link, ldr *loader.Loader, val int64) loader.Sym {
	idx := sort.Search(len(ctxt.Textp), func { i -> return ldr.SymValue(ctxt.Textp[i]) >= val })
