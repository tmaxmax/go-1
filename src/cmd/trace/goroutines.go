...
func GoroutinesHandlerFunc(summaries map[trace.GoID]*trace.GoroutineSummary) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		slices.SortFunc(groups, func { a, b | return cmp.Compare(b.ExecTime, a.ExecTime) })
...
func GoroutineHandler(summaries map[trace.GoID]*trace.GoroutineSummary) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if _, ok := validNonOverlappingStats[sortBy]; ok {
			slices.SortFunc(goroutines, func { a, b | return cmp.Compare(b.NonOverlappingStats[sortBy], a.NonOverlappingStats[sortBy]) })
		} else {
			// Sort by total time by default.
			slices.SortFunc(goroutines, func { a, b | return cmp.Compare(b.TotalTime, a.TotalTime) })
		}
