...
func UserTasksHandlerFunc(t *parsedTrace) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		slices.SortFunc(userTasks, func { a, b | return cmp.Compare(a.Type, b.Type) })
...
func UserTaskHandlerFunc(t *parsedTrace) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, summary := range t.summary.Tasks {
			...
			// Sort them.
			slices.SortStableFunc(rawEvents, func { a, b | return cmp.Compare(a.Time(), b.Time()) })
		...
		slices.SortFunc(tasks, func { a, b | return cmp.Compare(a.Duration, b.Duration) })
...
func newTaskFilter(r *http.Request) (*taskFilter, error) {
	...
	if typ, ok := param["type"]; ok && len(typ) > 0 {
		...
		conditions = append(conditions, func { _, task | return task.Name == typ[0] })
	}
	if complete := r.FormValue("complete"); complete == "1" {
		name = append(name, "complete")
		conditions = append(conditions, func { _, task | return task.Complete() })
	} else if complete == "0" {
	...
	if lat, err := time.ParseDuration(r.FormValue("latmin")); err == nil {
		...
		conditions = append(conditions, func { t, task | return task.Complete() && taskInterval(t, task).duration() >= lat })
	}
	...
	if text := r.FormValue("logtext"); text != "" {
		...
		conditions = append(conditions, func { _, task | return taskMatches(task, text) })
	}
