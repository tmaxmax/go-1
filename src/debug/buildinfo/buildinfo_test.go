...
func TestReadFile(t *testing.T) {
	...
	cleanOutputForComparison := func(got string) string {
		...
		got = buildRe.ReplaceAllStringFunc(got, func { match |
	...
	for _, p := range platforms {
		p := p
		t.Run(p.goos+"_"+p.goarch, func { t |
...
func FuzzIssue57002(f *testing.F) {
	...
	f.Fuzz(func { t, input | buildinfo.Read(bytes.NewReader(input)) })
}
...
func TestIssue54968(t *testing.T) {
	...
	for i := 1; i < paddingSize-len(buildInfoMagic); i++ {
		...
		t.Run(fmt.Sprintf("start_at_%d", i), func { t |
			d := data[:start]
